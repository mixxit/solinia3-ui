...
public class OceanMonumentPieces {
   ...
   public static class DoubleXRoom extends OceanMonumentPieces.Piece {
      ...
      public boolean addComponentParts(IWorld worldIn, Random randomIn, MutableBoundingBox structureBoundingBoxIn, ChunkPos p_74875_4_) {
         ...
         if (this.roomDefinition.index / 25 > 0) {
            this.generateDefaultFloor(worldIn, structureBoundingBoxIn, 8, 0, oceanmonumentpieces$roomdefinition.hasOpening[EnumFacing.DOWN.getIndex()]);
            this.generateDefaultFloor(worldIn, structureBoundingBoxIn, 0, 0, oceanmonumentpieces$roomdefinition1.hasOpening[EnumFacing.DOWN.getIndex()]);
         }
   ...
   public static class DoubleYZRoom extends OceanMonumentPieces.Piece {
      ...
      public boolean addComponentParts(IWorld worldIn, Random randomIn, MutableBoundingBox structureBoundingBoxIn, ChunkPos p_74875_4_) {
         ...
         if (this.roomDefinition.index / 25 > 0) {
            this.generateDefaultFloor(worldIn, structureBoundingBoxIn, 0, 8, oceanmonumentpieces$roomdefinition.hasOpening[EnumFacing.DOWN.getIndex()]);
            this.generateDefaultFloor(worldIn, structureBoundingBoxIn, 0, 0, oceanmonumentpieces$roomdefinition1.hasOpening[EnumFacing.DOWN.getIndex()]);
         }
   ...
   public abstract static class Piece extends StructurePiece {
      ...
      protected void makeOpening(IWorld worldIn, MutableBoundingBox boundingBoxIn, int x1, int y1, int z1, int x2, int y2, int z2) {
         for(int i = y1; i <= y2; ++i) {
            for(int j = x1; j <= x2; ++j) {
               for(int k = z1; k <= z2; ++k) {
                  IBlockState iblockstate = this.getBlockStateFromPos(worldIn, j, i, k, boundingBoxIn);
                  if (!field_212180_g.contains(iblockstate.getBlock())) {
                     if (this.getYWithOffset(i) >= worldIn.getSeaLevel() && iblockstate != WATER) {
      ...
      protected boolean spawnElder(IWorld worldIn, MutableBoundingBox p_175817_2_, int p_175817_3_, int p_175817_4_, int p_175817_5_) {
         ...
         if (p_175817_2_.isVecInside(new BlockPos(i, j, k))) {
            EntityElderGuardian entityelderguardian = new EntityElderGuardian(worldIn.getWorld());
            ...
            entityelderguardian.onInitialSpawn(worldIn.getDifficultyForLocation(new BlockPos(entityelderguardian)), (IEntityLivingData)null, (NBTTagCompound)null);
         ...
         for(int i = 1; i <= 6; ++i) {
            for(int j = 1; j <= 6; ++j) {
               if (randomIn.nextInt(3) != 0) {
                  ...
                  this.fillWithBlocks(worldIn, structureBoundingBoxIn, i, k, j, i, 3, j, iblockstate, iblockstate, false);
               }
            }
         }
   ...
   static class XYDoubleRoomFitHelper implements OceanMonumentPieces.MonumentRoomFitHelper {
      ...
      public boolean fits(OceanMonumentPieces.RoomDefinition definition) {
         if (definition.hasOpening[EnumFacing.EAST.getIndex()] && !definition.connections[EnumFacing.EAST.getIndex()].claimed && definition.hasOpening[EnumFacing.UP.getIndex()] && !definition.connections[EnumFacing.UP.getIndex()].claimed) {
            OceanMonumentPieces.RoomDefinition oceanmonumentpieces$roomdefinition = definition.connections[EnumFacing.EAST.getIndex()];
   ...
   static class YZDoubleRoomFitHelper implements OceanMonumentPieces.MonumentRoomFitHelper {
      ...
      public boolean fits(OceanMonumentPieces.RoomDefinition definition) {
         if (definition.hasOpening[EnumFacing.NORTH.getIndex()] && !definition.connections[EnumFacing.NORTH.getIndex()].claimed && definition.hasOpening[EnumFacing.UP.getIndex()] && !definition.connections[EnumFacing.UP.getIndex()].claimed) {
            OceanMonumentPieces.RoomDefinition oceanmonumentpieces$roomdefinition = definition.connections[EnumFacing.NORTH.getIndex()];
            return oceanmonumentpieces$roomdefinition.hasOpening[EnumFacing.UP.getIndex()] && !oceanmonumentpieces$roomdefinition.connections[EnumFacing.UP.getIndex()].claimed;
         } else {
            return false;
         }
