...
public class ClearCommand {
   ...
   private static int clearInventory(CommandSource source, Collection<EntityPlayerMP> targetPlayers, Predicate<ItemStack> itemPredicateIn, int maxCount) throws CommandSyntaxException {
      int i = 0;

      for(EntityPlayerMP entityplayermp : targetPlayers) {
         i += entityplayermp.inventory.clearMatchingItems(itemPredicateIn, maxCount);
      }
      ...
      if (i == 0) {
         ...
         if (maxCount == 0) {
            if (targetPlayers.size() == 1) {
               source.sendFeedback(new TextComponentTranslation("commands.clear.test.single", i, targetPlayers.iterator().next().getDisplayName()), true);
            } else {
               source.sendFeedback(new TextComponentTranslation("commands.clear.test.multiple", i, targetPlayers.size()), true);
            }
         } else if (targetPlayers.size() == 1) {
            source.sendFeedback(new TextComponentTranslation("commands.clear.success.single", i, targetPlayers.iterator().next().getDisplayName()), true);
         } else {
            source.sendFeedback(new TextComponentTranslation("commands.clear.success.multiple", i, targetPlayers.size()), true);
         }
