...
public class ScoreboardCommand {
   ...
   public static void register(CommandDispatcher<CommandSource> dispatcher) {
      dispatcher.register(Commands.literal("scoreboard").requires((p_198650_0_) -> {
         ...
         return addObjective(p_198636_0_.getSource(), StringArgumentType.getString(p_198636_0_, "objective"), ObjectiveCriteriaArgument.getObjectiveCriteria(p_198636_0_, "criteria"), new TextComponentString(StringArgumentType.getString(p_198636_0_, "objective")));
   ...
   private static int getPlayerScore(CommandSource source, String player, ScoreObjective objective) throws CommandSyntaxException {
      ...
      if (!scoreboard.entityHasObjective(player, objective)) {
         ...
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.get.success", player, score.getScorePoints(), objective.func_197890_e()), false);
   ...
   private static int applyScoreOperation(CommandSource source, Collection<String> targetEntities, ScoreObjective targetObjectives, OperationArgument.IOperation operation, Collection<String> sourceEntities, ScoreObjective sourceObjective) throws CommandSyntaxException {
      ...
      if (targetEntities.size() == 1) {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.operation.success.single", targetObjectives.func_197890_e(), targetEntities.iterator().next(), i), true);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.operation.success.multiple", targetObjectives.func_197890_e(), targetEntities.size()), true);
      }
   ...
   private static int enableTrigger(CommandSource source, Collection<String> targets, ScoreObjective objective) throws CommandSyntaxException {
      if (objective.getCriteria() != ScoreCriteria.TRIGGER) {
         ...
         if (i == 0) {
            ...
            if (targets.size() == 1) {
               source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.enable.success.single", objective.func_197890_e(), targets.iterator().next()), true);
            } else {
               source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.enable.success.multiple", objective.func_197890_e(), targets.size()), true);
            }
   ...
   private static int resetPlayerAllScores(CommandSource source, Collection<String> targets) {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.reset.all.single", targets.iterator().next()), true);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.reset.all.multiple", targets.size()), true);
      }
   ...
   private static int resetPlayerScore(CommandSource source, Collection<String> targets, ScoreObjective objective) {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.reset.specific.single", objective.func_197890_e(), targets.iterator().next()), true);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.reset.specific.multiple", objective.func_197890_e(), targets.size()), true);
      }
   ...
   private static int setPlayerScore(CommandSource source, Collection<String> targets, ScoreObjective objective, int newValue) {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.set.success.single", objective.func_197890_e(), targets.iterator().next(), newValue), true);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.set.success.multiple", objective.func_197890_e(), targets.size(), newValue), true);
      }
   ...
   private static int addToPlayerScore(CommandSource source, Collection<String> targets, ScoreObjective objective, int amount) {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.add.success.single", amount, objective.func_197890_e(), targets.iterator().next(), i), true);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.add.success.multiple", amount, objective.func_197890_e(), targets.size()), true);
      }
   ...
   private static int removeFromPlayerScore(CommandSource source, Collection<String> targets, ScoreObjective objective, int amount) {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.remove.success.single", amount, objective.func_197890_e(), targets.iterator().next(), i), true);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.remove.success.multiple", amount, objective.func_197890_e(), targets.size()), true);
      }
   ...
   private static int listPlayers(CommandSource source) {
      ...
      if (collection.isEmpty()) {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.list.empty"), false);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.list.success", collection.size(), TextComponentUtils.makeGreenSortedList(collection)), false);
      }
   ...
   private static int listPlayerScores(CommandSource source, String player) {
      ...
      if (map.isEmpty()) {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.list.entity.empty", player), false);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.list.entity.success", player, map.size()), false);

         for(Entry<ScoreObjective, Score> entry : map.entrySet()) {
            source.sendFeedback(new TextComponentTranslation("commands.scoreboard.players.list.entity.entry", entry.getKey().func_197890_e(), entry.getValue().getScorePoints()), false);
         }
      }
   ...
   private static int clearObjectiveDisplaySlot(CommandSource source, int slotId) throws CommandSyntaxException {
      ...
      if (scoreboard.getObjectiveInDisplaySlot(slotId) == null) {
         ...
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.objectives.display.cleared", Scoreboard.getDisplaySlotStrings()[slotId]), true);
         return 0;
      }
   }
   ...
   private static int setObjectiveDisplaySlot(CommandSource source, int slotId, ScoreObjective objective) throws CommandSyntaxException {
      ...
      if (scoreboard.getObjectiveInDisplaySlot(slotId) == objective) {
         ...
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.objectives.display.set", Scoreboard.getDisplaySlotStrings()[slotId], objective.getDisplayName()), true);
         return 0;
      }
   }
   ...
   private static int setDisplayName(CommandSource source, ScoreObjective objective, ITextComponent displayName) {
      if (!objective.getDisplayName().equals(displayName)) {
         ...
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.objectives.modify.displayname", objective.getName(), objective.func_197890_e()), true);
      }
   ...
   private static int setRenderType(CommandSource source, ScoreObjective objective, ScoreCriteria.RenderType renderType) {
      if (objective.getRenderType() != renderType) {
         ...
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.objectives.modify.rendertype", objective.func_197890_e()), true);
      }
   ...
   private static int removeObjective(CommandSource source, ScoreObjective objective) {
      ...
      source.sendFeedback(new TextComponentTranslation("commands.scoreboard.objectives.remove.success", objective.func_197890_e()), true);
   ...
   private static int addObjective(CommandSource source, String name, ScoreCriteria criteria, ITextComponent displayName) throws CommandSyntaxException {
      ...
      if (scoreboard.getObjective(name) != null) {
         ...
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.objectives.add.success", scoreobjective.func_197890_e()), true);
         return scoreboard.getScoreObjectives().size();
      }
   }
   ...
   private static int listObjectives(CommandSource source) {
      ...
      if (collection.isEmpty()) {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.objectives.list.empty"), false);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.scoreboard.objectives.list.success", collection.size(), TextComponentUtils.makeList(collection, ScoreObjective::func_197890_e)), false);
      }
