...
public class ReplaceItemCommand {
   ...
   private static int replaceItemBlock(CommandSource source, BlockPos pos, int slotIn, ItemStack newStack) throws CommandSyntaxException {
      ...
      if (!(tileentity instanceof IInventory)) {
         ...
         if (slotIn >= 0 && slotIn < iinventory.getSizeInventory()) {
            ...
            source.sendFeedback(new TextComponentTranslation("commands.replaceitem.block.success", pos.getX(), pos.getY(), pos.getZ(), newStack.getTextComponent()), true);
            return 1;
         } else {
   ...
   private static int replaceItemEntities(CommandSource source, Collection<? extends Entity> targets, int slotIn, ItemStack newStack) throws CommandSyntaxException {
      ...
      if (list.isEmpty()) {
         ...
         if (list.size() == 1) {
            source.sendFeedback(new TextComponentTranslation("commands.replaceitem.entity.success.single", list.iterator().next().getDisplayName(), newStack.getTextComponent()), true);
         } else {
            source.sendFeedback(new TextComponentTranslation("commands.replaceitem.entity.success.multiple", list.size(), newStack.getTextComponent()), true);
         }
