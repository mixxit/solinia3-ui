...
public class RecipeCommand {
   ...
   private static int giveRecipes(CommandSource source, Collection<EntityPlayerMP> targets, Collection<IRecipe> recipes) throws CommandSyntaxException {
      ...
      if (i == 0) {
         ...
         if (targets.size() == 1) {
            source.sendFeedback(new TextComponentTranslation("commands.recipe.give.success.single", recipes.size(), targets.iterator().next().getDisplayName()), true);
         } else {
            source.sendFeedback(new TextComponentTranslation("commands.recipe.give.success.multiple", recipes.size(), targets.size()), true);
         }
   ...
   private static int takeRecipes(CommandSource source, Collection<EntityPlayerMP> targets, Collection<IRecipe> recipes) throws CommandSyntaxException {
      ...
      if (i == 0) {
         ...
         if (targets.size() == 1) {
            source.sendFeedback(new TextComponentTranslation("commands.recipe.take.success.single", recipes.size(), targets.iterator().next().getDisplayName()), true);
         } else {
            source.sendFeedback(new TextComponentTranslation("commands.recipe.take.success.multiple", recipes.size(), targets.size()), true);
         }
