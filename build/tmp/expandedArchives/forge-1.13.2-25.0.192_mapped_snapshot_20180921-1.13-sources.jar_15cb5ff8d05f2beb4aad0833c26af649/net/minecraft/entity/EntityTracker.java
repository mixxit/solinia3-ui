...
public class EntityTracker {
   ...
   public void track(Entity entityIn) {
      if (entityIn.getType().hasCustomTracking()) {
         this.track(entityIn, entityIn.getType().getTrackingRange(), entityIn.getType().getUpdateFrequency(), entityIn.getType().shouldSendVelocityUpdates());
         return;
      }
   ...
   public void track(Entity entityIn, int trackingRange, int updateInterval) {
      this.track(entityIn, trackingRange, updateInterval, false);
   }
   ...
   public void track(Entity entityIn, int trackingRange, int updateInterval, boolean sendVelocityUpdates) {
      try {
         ...
         EntityTrackerEntry entitytrackerentry = new EntityTrackerEntry(entityIn, trackingRange, this.maxTrackingDistanceThreshold, updateInterval, sendVelocityUpdates);
         ...
         this.trackedEntityHashTable.lookup(entityIn.getEntityId()).getTrackedEntity().fillCrashReport(crashreport.makeCategory("Entity That Is Already Tracked"));
