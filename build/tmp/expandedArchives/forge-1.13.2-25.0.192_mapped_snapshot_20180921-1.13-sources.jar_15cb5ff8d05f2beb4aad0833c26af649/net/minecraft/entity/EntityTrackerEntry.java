...
public class EntityTrackerEntry {
   ...
   /**
    * every 400 ticks a  full teleport packet is sent, rather than just a "move me +x" command, so that position remains
    * fully synced.
    */
   ...
   public void tick(List<EntityPlayer> players) {
   ...
   public void updatePlayers(List<EntityPlayer> players) {
   ...
   private Packet<?> createSpawnPacket() {
      ...
      Packet<?> pkt = net.minecraftforge.fml.network.NetworkHooks.getEntitySpawningPacket(this.trackedEntity);
      ...
      if (this.trackedEntity instanceof EntityPlayerMP) {
         ...
         return new SPacketSpawnObject(this.trackedEntity, 70, Block.getStateId(entityfallingblock.getBlockState()));
