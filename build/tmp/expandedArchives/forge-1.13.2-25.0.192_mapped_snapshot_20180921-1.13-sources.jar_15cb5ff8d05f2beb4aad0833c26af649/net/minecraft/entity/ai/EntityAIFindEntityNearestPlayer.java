...
public class EntityAIFindEntityNearestPlayer extends EntityAIBase {
   ...
   public EntityAIFindEntityNearestPlayer(EntityLiving entityLivingIn) {
      ...
      this.predicate = (p_210293_1_) -> {
         if (!(p_210293_1_ instanceof EntityPlayer)) {
            return false;
         } else if (((EntityPlayer)p_210293_1_).abilities.disableDamage) {
            return false;
         } else {
   ...
   public boolean shouldExecute() {
      double d0 = this.maxTargetRange();
      List<EntityPlayer> list = this.entityLiving.world.getEntitiesWithinAABB(EntityPlayer.class, this.entityLiving.getBoundingBox().grow(d0, 4.0D, d0), this.predicate);
