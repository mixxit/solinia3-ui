...
public class GuiSpellbookButton extends Button{
	protected static final ResourceLocation UTILITYSPELLBOOK = new ResourceLocation( solinia3ui.MOD_ID, "textures/gui/utility.png");
	...
	public GuiSpellbookButton(int widthIn, int heightIn, int x, int y, String text, Button.IPressable onPress) {
		super(widthIn, heightIn, x, y, text, onPress);
		this.minecraft = Minecraft.getInstance();
		
		popupText = Lists.newArrayList();
		popupText.add("Drop spell icons here");
		popupText.add("or click to open spellbook");
	...
	public SpellIconLocation setSpellIconResourceLocations() {
		for (int spellIconSheet = 0; spellIconSheet < 100; spellIconSheet++)
        {
            ...
            while ((_y + this.height) < _spellFileHeightArea)
            {
            	...
            	this.spellIconLocations.put(foundNewIconId,new SpellIconLocation(UTILITYSPELLBOOK,new Point(_x,_y)));
